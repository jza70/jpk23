...
fn test_invalid_root_namespace_fails() {
    ...
    let mut output = Vec::new();
    let result = process_jpk(Cursor::new(input), &mut output, None, None, FormVariant::Unknown);
...
fn test_namespace_override() {
    ...
    let mut output = Vec::new();
    process_jpk(Cursor::new(input), &mut output, Some("foo".to_string()), None, FormVariant::Unknown).unwrap();
    let result = String::from_utf8(output).unwrap();
...
fn test_namespace_strip() {
    ...
    let mut output = Vec::new();
    process_jpk(Cursor::new(input), &mut output, Some("".to_string()), None, FormVariant::Unknown).unwrap();
    let result = String::from_utf8(output).unwrap();
